...
List<Map> getVersesTextByPage(int pageNumber,
    {bool verseEndSymbol = false,
  ...
  for (var data in pageData) {
    if (data["start"] == 1) {
      if (customSurahSeperator != "") {
        verses.add({"customSurahSeperator": customSurahSeperator});
      } else if (surahSeperator == SurahSeperator.surahName) {
        verses.add({"surahSeperator": getSurahName(data["surah"])});
      } else if (surahSeperator == SurahSeperator.surahNameArabic) {
        verses.add({"surahSeperator": getSurahNameArabic(data["surah"])});
      } else if (surahSeperator == SurahSeperator.surahNameEnglish) {
        verses.add({"surahSeperator": getSurahNameEnglish(data["surah"])});
      } else if (surahSeperator == SurahSeperator.surahNameWithBasmala) {
        verses.add({"surahSeperator": getSurahNameArabic(data["surah"])});
      }
    }
    for (int j = data["start"]; j <= data["end"]; j++) {
      verses.add({
        "verse": getVerse(data["surah"], j, verseEndSymbol: verseEndSymbol),
        "surah": data["surah"],
        "id": j
      });
    }
